...
class SgaFunction(Function):
    ...
    def forward(ctx, input, g0, g1, g2, g3):
        assert (input.is_contiguous() == True and g0.is_contiguous() == True
                and g1.is_contiguous() == True and g2.is_contiguous() == True)
    ...
    def backward(ctx, gradOutput):
        ...
        with torch.cuda.device_of(gradOutput):
            ...
            GANet.sga_cuda_forward(input, g0, g1, g2, g3, temp_out, mask, max_idx, gradOutput,
...
class Lga3Function(Function):
    ...
    def backward(ctx, gradOutput):
        ...
        with torch.cuda.device_of(gradOutput):
            ...
            GANet.lga_cuda_forward(temp_out2, filters, gradOutput, temp_out2, gradFilters, ctx.radius)
            GANet.lga_cuda_forward(temp_out1, filters, temp_out2, temp_out1, gradFilters, ctx.radius)
            GANet.lga_cuda_forward(input, filters, temp_out1, temp_out2, gradFilters, ctx.radius)
...
class Lga2Function(Function):
    ...
    def backward(ctx, gradOutput):
        ...
        with torch.cuda.device_of(gradOutput):
            num, channels, height, width = input.size()
            _, fsize, _, _ = filters.size()
            gradFilters = gradOutput.new().resize(num, fsize, height, width).zero_()
            GANet.lga_cuda_forward(temp_out, filters, gradOutput, temp_out, gradFilters, ctx.radius)
            GANet.lga_cuda_forward(input, filters, temp_out, gradOutput, gradFilters, ctx.radius)
            temp_out[...] = gradOutput[...]
