...
class Feature(nn.Module):
    ...
    def forward(self, x):
        x = self.conv_start(x)  # (w, h) / 3
        rem0 = x
        x = self.conv1a(x)  # (w, h) / 6
        rem1 = x
        x = self.conv2a(x)  # (w, h) / 12
        rem2 = x
        x = self.conv3a(x)  # (w, h) / 24
        rem3 = x
        x = self.conv4a(x)  # (w, h) / 48
        rem4 = x
        x = self.deconv4a(x, rem3)  # (w, h) / 24
        rem3 = x
        x = self.deconv3a(x, rem2)  # (w, h) / 12
        rem2 = x
        x = self.deconv2a(x, rem1)  # (w, h) / 6
        rem1 = x
        x = self.deconv1a(x, rem0)  # (w, h) / 3
        rem0 = x

        x = self.conv1b(x, rem1)  # (w, h) / 6
        rem1 = x
        x = self.conv2b(x, rem2)  # (w, h) / 12
        rem2 = x
        x = self.conv3b(x, rem3)  # (w, h) / 24
        rem3 = x
        x = self.conv4b(x, rem4)  # (w, h) / 48

        x = self.deconv4b(x, rem3)  # (w, h) / 24
        x = self.deconv3b(x, rem2)  # (w, h) / 12
        x = self.deconv2b(x, rem1)  # (w, h) / 6
        x = self.deconv1b(x, rem0)  # (w, h) / 3
...
class Guidance(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(BasicConv(16, 32, kernel_size=5, stride=3, padding=2),
                                   BasicConv(32, 32, kernel_size=3, padding=1))
        ...
        self.weight_sgl1 = nn.Conv2d(48, 960, (3, 3), (1, 1), (1, 1), bias=False)
        self.weight_sgl2 = nn.Conv2d(48, 960, (3, 3), (1, 1), (1, 1), bias=False)
    ...
    def forward(self, x):
        ...
        x = self.conv11(x)
        sg11 = self.weight_sgl1(x)
        x = self.conv12(x)
        sg12 = self.weight_sgl2(x)
...
#        self.conv32x1 = BasicConv(32, 1, kernel_size=3)
    ...
    def forward(self, x):
        x = F.interpolate(self.conv32x1(x), [self.max_disp + 1, x.size()[3] * 3, x.size()[4] * 3], mode='trilinear',
                          align_corners=False)
        x = torch.squeeze(x, 1)
        x = self.softmax(x)
...
class CostAggregation(nn.Module):
    def __init__(self, max_disp=192):
        ...
        self.disp0 = Disp(self.max_disp)
        self.disp1 = DispAgg(self.max_disp)
...
class GANet(nn.Module):
    def __init__(self, max_disp=192):
        ...
        self.cost_agg = CostAggregation(self.max_disp)
        self.cv = GetCostVolume(int(self.max_disp / 3))
